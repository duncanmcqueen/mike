...
The model menu includes built-in cloud models and every model or committee in `MIKE_MODEL_CONFIG_JSON`. A local OpenAI-compatible model works without a provider API key when its configured endpoint does not require one.
