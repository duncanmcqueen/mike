...
import { SETTINGS_MODELS } from "@/app/components/assistant/ModelToggle";
import {
  ...
  type PlaybookPosition, type PlaybookRule, type PlaybookRun,
...
export default function PlaybooksPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPlaybooks(items);
      const available = new Set(configuration.availableModelIds);
      const merged = new Map(SETTINGS_MODELS.map((entry) => [entry.id, { value: entry.id, label: `${entry.group} · ${entry.label}` }]));
      for (const entry of configuration.configuredModels) merged.set(entry.id, { value: entry.id, label: `${entry.location === "committee" ? "Committee" : entry.location === "local" ? "Local" : "Cloud"} · ${entry.label}` });
      const selectable = [...merged.values()].filter((entry) => available.has(entry.value));
      setModels(selectable);
      setModel((current) => available.has(current)
        ? current
        : configuration.defaultModel && available.has(configuration.defaultModel)
  ...
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <Modal open={importOpen} onClose={() => setImportOpen(false)} size="sm" breadcrumbs={["Playbooks", "Import Word playbook"]} primaryAction={{ label: busy === "import" ? "Compiling…" : "Import and compile", disabled: !importFile || !model || !!busy, onClick: () => void doImport() }} cancelAction={{ label: "Cancel", onClick: () => setImportOpen(false) }}><div className="space-y-4 overflow-y-auto pb-4"><p className="text-sm text-gray-600">The imported concepts, positions, and sample clauses remain a draft until you review and publish them.</p><div><ModalFieldLabel>Playbook name (optional)</ModalFieldLabel><ModalTextInput value={importName} onChange={(event) => setImportName(event.target.value)} /></div><div><ModalFieldLabel>Word playbook</ModalFieldLabel><input type="file" accept=".docx" onChange={(event) => setImportFile(event.target.files?.[0] ?? null)} className="block w-full text-sm" /></div><div><ModalFieldLabel>Compilation model</ModalFieldLabel>{models.length > 0 ? <ModalSelect id="playbook-import-model" value={model} onChange={setModel} options={models} /> : <p className="text-sm text-amber-700">No compilation model is available. <a href="/account/api-keys" className="underline">Configure an API key</a> or enable a local model.</p>}</div></div></Modal>
    <Modal open={reviewOpen} onClose={() => setReviewOpen(false)} size="sm" breadcrumbs={["Playbooks", "Review document"]} primaryAction={{ label: busy === "review" ? "Reviewing…" : "Start review", disabled: !reviewFile || !model || !!busy, onClick: () => void review() }} cancelAction={{ label: "Cancel", onClick: () => setReviewOpen(false) }}><div className="space-y-4 overflow-y-auto pb-4"><div><ModalFieldLabel>Contract</ModalFieldLabel><input type="file" accept=".docx,.txt" onChange={(event) => setReviewFile(event.target.files?.[0] ?? null)} className="block w-full text-sm" /></div><div><ModalFieldLabel>Review model</ModalFieldLabel>{models.length > 0 ? <ModalSelect id="playbook-review-model" value={model} onChange={setModel} options={models} /> : <p className="text-sm text-amber-700">No review model is available. <a href="/account/api-keys" className="underline">Configure an API key</a> or enable a local model.</p>}</div><div><ModalFieldLabel>Review posture</ModalFieldLabel><ModalSelect id="playbook-review-mode" value={reviewMode} onChange={(value) => setReviewMode(value as "strict" | "permissive")} options={[{ value: "strict", label: "Strict — push standard positions" }, { value: "permissive", label: "Permissive — allow fallbacks" }]} /></div></div></Modal>
  </div>;
}
